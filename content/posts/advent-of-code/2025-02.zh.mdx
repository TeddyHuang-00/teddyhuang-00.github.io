...
2. 使用容斥原理，去除重复计算的部分。由于质数之间互质，唯一的可能重叠计算部分即为长度为 $1$ 的片段，因此我们只需从总和中减去长度为 $1$ 的部分即可。

如此一来，我们不仅确保了结果的正确性，还保持了较高的计算效率。

## 核心算法实现

```rust title="day02/src/main.rs" {"提取所有质因数":1-21} {"高斯求和":41-42} {"找出数字位数范围": 50-51,66-67} {"容斥原理去重": 77-81}
///
