...
值得注意的是，在本题中，由于每个点的坐标均为整数，我们实际上可以使用 Morton 编码，也即[<Ruby rt="Z-order| |curve" rb="Z-序||曲线" split="|" />](https://en.wikipedia.org/wiki/Z-order_curve)来对空间进行划分。Morton 编码通过将每个坐标的二进制位交错排列，生成一个一维的编码值，从而实现空间的线性化表示，如下图所示：
...
在有了八叉树结构后，我们可以利用空间划分来加速最近邻搜索，也就是[<Ruby rt="branch| and |bound" rb="分支||定界" split="|" />](https://zh.wikipedia.org/wiki/分支定界)。在搜索的过程中，我们在已经得到了若干个最近邻候选点后，可以利用当前的最短距离来剪枝八叉树中的节点。具体来说，假设当前最近邻距离为 $d$，我们在搜索过程中遇到一个八叉树节点时，可以计算该节点所覆盖的空间与查询点之间的最小距离 $d_{\min}$。如果 $d_{\min} \geq d$，则说明该节点及其子节点中的所有点都不可能是更近的邻居，因此可以直接跳过该节点，避免不必要的计算。
...
在计算 $d_{\min}$ 时，我们可以利用 Morton 编码的性质来快速估计两个节点覆盖空间之间的最小距离。具体来说，每个节点对应一个 Morton 编码值的前缀，其所有子节点对应的 Morton 编码值均以该前缀开头。因此我们可以从该前缀中提取出对应的坐标范围，从而得到该节点对应的[**<Ruby rt="Axis-Aligned| |Bounding Box" rb="按坐标轴排列的||包围盒" split="|" />**（AABB）](https://zh.wikipedia.org/wiki/包围体)。然后，我们可以使用 AABB 之间的距离计算方法在 $O(1)$ 时间内得到 $d_{\min}$。
