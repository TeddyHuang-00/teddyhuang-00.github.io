...
给定 $N$ 个三维空间内的点，每个点坐标 $(x, y, z)$ 均为非负整数。同时确保每个点坐标唯一，且任意两个点之间的欧氏距离[^1]互不相等。试计算：
...
不难发现，第一问中我们可以用并查集[^2]来管理连通分量，而第二问中我们也同样需要使用并查集来检查点的连接状态。这并不是什么难事，因此最大的挑战均在于每个问题的第一步：如何高效地找到所有点对之间的前 $K$ 短距离或者将点按距离顺序连接起来。

### 并行优化

对于找寻前 $K$ 短距离以及前 $3$ 大连通分量，我们可以用最大/最小堆[^3]来辅助计算。当然，最耗时的步骤是计算所有点对之间的距离。为此，我们可以使用一些简单的并行优化手段：
...
<Ruby rt="Reduction" rb="归约" /> 是一种常见的并行计算模式，对于 $M$ 个类型为
$A$ 的输入数据，通过一个二元操作符 $f: A\times A\to A$，将其归约为一个类型为 $A$
的输出结果。归约操作通常满足结合律，即对于任意的 $a, b, c$，都有 $f(f(a, b), c)
\equiv f(a, f(b,
c))$。比如在本题中，我们合并两个堆时，输出的仍是一个堆，并且满足结合律。对于这样的操作，我们可以将输入数据划分为若干份，分别在多个线程中进行归约操作，常见的模式是如下图所示的树形结构：
...
值得注意的是，在本题中，由于每个点的坐标均为整数，我们实际上可以使用 Morton 编码，也即 <Ruby rt="Z-order |curve" rb="Z-序|曲线" split="|" />[^4] 来对空间进行划分。Morton 编码通过将每个坐标的二进制位交错排列，生成一个一维的编码值，从而实现空间的线性化表示，如下图所示：
...
在有了八叉树结构后，我们可以利用空间划分来加速最近邻搜索，也就是 <Ruby rt="branch |and |bound" rb="分支||定界" split="|" /> [^5]。在搜索的过程中，我们在已经得到了若干个最近邻候选点后，可以利用当前的最短距离来剪枝八叉树中的节点。具体来说，假设当前最近邻距离为 $d$，我们在搜索过程中遇到一个八叉树节点时，可以计算该节点所覆盖的空间与查询点之间的最小距离 $d_{\min}$。如果 $d_{\min} \geq d$，则说明该节点及其子节点中的所有点都不可能是更近的邻居，因此可以直接跳过该节点，避免不必要的计算。

由于在这两问中，我们都需要考虑**所有点之间**的距离，因此我们可以改用 <Ruby rt="dual-tree| |traversal" rb="双树||遍历" split="|" /> 的方法来批次处理点对之间的计算。比如以第一问为例，我们可以将所有点构建为八叉树的根节点，然后遍历：
...
在计算 $d_{\min}$ 时，我们可以利用 Morton 编码的性质来快速估计两个节点覆盖空间之间的最小距离。具体来说，每个节点对应一个 Morton 编码值的前缀，其所有子节点对应的 Morton 编码值均以该前缀开头。因此我们可以从该前缀中提取出对应的坐标范围，从而得到该节点对应的 **<Ruby rt="Axis-Aligned |Bounding Box" rb="按坐标轴排列的|包围盒" split="|" />**（AABB [^6]）。然后，我们可以使用 AABB 之间的距离计算方法在 $O(1)$ 时间内得到 $d_{\min}$。

<Aside type="caution" title="前缀估计的 AABB 与实际 AABB">

由前缀估计得到的 AABB 为理论上最大的包围盒，实际节点所包含的点集可能分布得更紧凑，从而导致实际 AABB 更小。当点集分布较为均匀时，这种差异通常较小；但当点集分布极为不均匀时，差异可能会很大，从而影响剪枝效果。因此，在实际实现中，我们可以在构建八叉树时，预先计算出每个节点的实际 AABB，从而提升剪枝路径的命中率。
...
实际上，我们可以重新审视第二问的需求。在之前的分析中，我们已经得出结论：我们只需要关注那些连接不同连通分量的边。这个过程实际上就是 Kruskal 算法构建 <Ruby rt="Minimum |Spanning Tree" rb="最小|生成树" split="|" />（MST）的过程。而最后一条边则是这个最小生成树中的最长边。由于最小生成树的唯一性，如果我们可以直接构建最小生成树，那么我们就可以更快地找到这条边。

<Aside type="important" title="证明最小生成树的唯一性">

> 对于一个加权无向图 $\mathcal{G}$，如果所有边的权重均不相等，那么该图的最小生成树是唯一的。
...
4. 重复以下步骤直到栈为空：
   ...
   4. 如果 $L$ 和 $R$ 的大小低于某个阈值，则直接计算 $L$ 和 $R$ 中所有点对之间的距离，并更新每个连通分量的最短距离；
   5. 否则，按照以下规则产出新的节点对入栈：
      - 如果 $L$ 和 $R$ 是同一个节点，则产出所有不重复的子节点两两组合的节点对（$i < j$）；
      - 否则，分裂较大的节点，产出其子节点与另一个节点的所有组合的节点对。
...
[^2]: https://zh.wikipedia.org/wiki/并查集

[^3]: https://zh.wikipedia.org/wiki/二叉堆

[^4]: https://en.wikipedia.org/wiki/Z-order_curve

[^5]: https://zh.wikipedia.org/wiki/分支定界

[^6]: https://zh.wikipedia.org/wiki/包围体
