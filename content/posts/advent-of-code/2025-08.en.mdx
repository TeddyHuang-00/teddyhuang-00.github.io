...
Given $N$ points in three-dimensional space, where each point's coordinates $(x, y, z)$ are non-negative integers. It is also ensured that each point's coordinates are unique, and the Euclidean distances[^1] between any two points are all distinct. Compute:
...
It's not difficult to see that for the first part we can use a Disjoint Set Union (DSU)[^2] to manage connected components, and for the second part we similarly need to use DSU to check the connection status of points. This is not particularly challenging, so the main difficulty lies in the first step of each problem: how to efficiently find the first $K$ shortest distances among all point pairs, or to connect the points in distance order.

### Parallel Optimization

For finding the first $K$ shortest distances and the top $3$ largest connected components, we can use max/min heaps[^3] to assist the computation. Of course, the most time-consuming step is calculating the distances between all point pairs. To address this, we can employ some simple parallel optimization techniques:
...
[^6]: https://en.wikipedia.org/wiki/Bounding_volume
