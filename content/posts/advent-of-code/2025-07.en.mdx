...
In fact, observing the input reveals that splitters in the same row are separated by at least one empty cell, so they don't interfere with each other. This allows us to integrate the logic for checking if a cell is a splitter from the two branching steps:
